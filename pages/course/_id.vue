                  ...
                  <section class="mt20">
                    <div class="lh-menu-wrap">
                      <menu id="lh-menu" class="lh-menu mt10 mr10">
                        <ul>
                          <!-- 文件目录 -->
                          <li class="lh-menu-stair" v-for="item in subjectList" :key="item.id" :title="item.title">
                            <a href="javascript: void(0)" class="current-1">
                              <em class="lh-menu-i-1 icon18 mr10"></em>{{item.title}}
                            </a>
                            <ol class="lh-menu-ol" style="display: block;">
                              <li class="lh-menu-second ml30" v-for="item2 in item.list" :key="item2.id" :title="item2.title">
                                <!--  target="_blank"实现增加新的页面来跳转 -->
                                <a :href="'/player/'+item2.videoSourceId" :title="item2.title" target="_blank">
                                  <span class="fr">
                                    <i class="free-icon vam mr10">免费试听</i>
                                  </span>
                                  <em class="lh-menu-i-2 icon16 mr5">&nbsp;</em>{{item2.title}}
                                </a>
                              </li>
                            </ol>
                          </li>
...
export default {
  asyncData({params,error}){
    return course.findCourseById(params.id).then(Response=>{
      return {
        courseWebVo:Response.data.data.courseWebVo,
        subjectList:Response.data.data.subjectList
      }
    })
  },
  methods:{
    // 生成订单，跳转订单页面
    createOrder(){
      order.saveOrder(this.$route.params.id).then(Response=>{
        if(Response.data.success){
          // 订单创建成功，跳转到订单页面
          this.$router.push({path:'/order/'+Response.data.data.tOrderNo}) 
        } 
      })
    }
  }
